...
create_certs() {
  ...
  ipsec pki --gen --type rsa --size 2048 \
    --outform pem > /etc/ipsec.d/private/serverKey.pem  
  ...
  ipsec pki --gen --type rsa --size 2048 \
    --outform pem > /etc/ipsec.d/private/userKey.pem
  chmod 600 /etc/ipsec.d/private/userKey.pem
  ...
  ipsec pki --issue --lifetime 1095 \
    ...
    --san="$USER_CN" \
    --outform pem > /etc/ipsec.d/certs/userCert.pem
